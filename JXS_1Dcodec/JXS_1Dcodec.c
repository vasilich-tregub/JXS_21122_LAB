// JXS_1Dcodec.cpp : Defines the entry point for the application.
//
...
int main(int argc, char** argv)
{
	...
	xs_image_t image = { 0 };
	const char* output_fn = "fraunhofer-mono-mls.jxs";
	...
	fprintf(stderr, "Examining JPEG XS lossless packing\n");


	do
	{
		const uint32_t NLx = 5; // horizontal wavelet decomposition levels
		...
		for (int iy = 0; iy < height; ++iy) {
			for (int ix = 0; ix < width; ++ix) {
				ptr0[iy * width + ix] = (ix - (int)width / 2) * (1 << depth) / (int)width + (1 << (depth - 1)) /**/ + 1;   // Y
				//ptr0[iy * width + ix] += (int)(ix * ix * ix * ix / width / width / width); // nonlinear grad
				ptr0[iy * width + ix] = clamp(ptr0[iy * width + ix], (1 << depth));
			}
		}
		...
		xs_config.profile = XS_PROFILE_MLS_12; // XS_PROFILE_UNRESTRICTED;
		...
		xs_config.p.S_s = 8;
		xs_config.p.Bw = 255;
		xs_config.p.Fq = 0; // fractional bits; 8 in xs_enc_generated/monochrome; must be 0 in mls
		...
		xs_config.p.NLx = NLx; // horizontal wavelet decomposition levels
		xs_config.p.NLy = 0;
